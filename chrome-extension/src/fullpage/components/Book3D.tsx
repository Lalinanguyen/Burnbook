...
export type BookAnimMode = 'idle' | 'throwing' | 'burning';
...
const THROW_DURATION = 2.5;
const BURN_DURATION = 3.0;
const THROW_FLIGHT = 1.8;
const BURN_FADE = 2.5;
const GRAVITY = 18;
...
export default function Book3D({
  ...
  useFrame(() => {
    ...
    // Detect mode transitions and initialize
    ...
    const animDuration = mode === 'throwing' ? THROW_DURATION : BURN_DURATION;
    ...
    if (internalPhaseRef.current === 'anim') {
      if (mode === 'throwing') {
        if (elapsed < THROW_FLIGHT) {
          ...
          mat.opacity = Math.max(0, 1 - elapsed / THROW_FLIGHT);
        } else {
          mat.opacity = 0;
        }
      } else if (mode === 'burning') {
      ...
      // Respawn phase — book drops from above with bounce
